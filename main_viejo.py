...
def main():
    ...
    df_alt_correct_price, df_alt = format_data.main(df_alt)

    print("Exporto Dataframe alternativas con precio corregido pues sera utilizado para ser presentado al cliente \n")
    df_alt_correct_price.to_excel('/Users/nachomondino/Documents/GitHub/evaluacion-compra-automatica/data/data_preparation/{}/df_alt_correct_price.xlsx'.format(producto), index=False)  # cuando corra tod@ junto pongo product.nombre

    print("(3.2) CLEAN DATA ".center(120))
    df_alt_cleaned, df_opi, df_opi_tokenizado = clean_data.main(df_alt, df_opi)  # retorna df_opi porque debo borrar dupl, nan y fecha de emision (pero no tokenizar, ni nada)
    ...
    df_alt_cleaned, df_cust_needs, df_relation_matrix = construct_data.main(df_alt_cleaned, df_opi_tokenizado)
    l_cust_needs_one_word = list(df_cust_needs.index)  # customer needs de una palabra
    ...
    df_cust_need_sent, df_attr_values_sent = sentiment_atribution.main(df_alt_cleaned, df_opi, df_relation_matrix)
    ...
    df_alt, df_alt_per_clust, df_centroids_values, df_brand_per_cluster =  clustering.main(df_alt, df_alt_cleaned, df_attr_values_sent)
...
''' # EN DESUSO POR IS_PRODUCT_VALID
