...
def sample_analyze_entity_sentiment(text_content):
    ...
    for entity in response.entities:  #para no modificar el codigo, podria devolver diccionario con entities como key y lista de mag, score y salience de values
        ...
        if entidad not in data.keys():
            # Guardo entidad y su score
            # data[entidad] = [round(sentiment.score, 2), round(sentiment.magnitude, 2)]  #cdo usaba magnitude
            data[entidad] = round(sentiment.score, 2)

        # si la entidad es ya fue extraida para la opinion
        else:
            # las entidads ya estan ordenadas por salience, por lo que, no guardo datos de la repitcion
            pass
